...
import { MinTokenData, TransactionParams, createSwapper } from '~/utils/paraswap';
...
export const useGetSwapTransaction = (): UseMutationResult<TransactionParams, unknown, void> => {
  ...
  const getSwapTransaction = useCallback(async (): Promise<TransactionParams> => {
    ...
    const srcAmount = new BigNumber(youPay.amount).times(10 ** srcToken.decimals).toFixed(0);
    ...
    const minAmount = new BigNumber(priceRoute.destAmount).times(1 - DEFAULT_SLIPPAGE / 100).toFixed(0);
